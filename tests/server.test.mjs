...
test('two clients: readiness gate, zero-axis shot, jumping origin, loadout lock and room cleanup', { timeout: 20000 }, async t => {
  ...
  const briefingEvent = once(host, 'briefing'); host.emit('startMatch');
  const [briefing] = await briefingEvent;
  assert.equal((await request(host, 'shoot', { dx: 0, dy: 0, dz: -1 })).ok, false);
  await delay(150); assert.equal(starts, 0);
  host.emit('briefingReady', { id: briefing.id });
  await delay(100); assert.equal(starts, 0);
  // Wrong briefing ids cannot trigger deployment.
  guest.emit('briefingReady', { id: briefing.id - 1 });
  await delay(100); assert.equal(starts, 0);
  const started = once(host, 'matchStart');
  guest.emit('briefingReady', { id: briefing.id });
  const [match] = await started;
  assert.ok(match.endsAt - Date.now() > 359000);
  assert.equal(match.players.length, 2);
  host.emit('input', { seq: 1, x: -3.4, y: .5, z: 4.6, yaw: 0, pitch: 0 });
  ...
  host.emit('input',{seq:2,x:0,y:2.4,z:-13,yaw:0,pitch:0});
  ...
  const readyAgain = once(host, 'lobby');
  host.emit('setLoadout', { ready: true }); await readyAgain;
  const nextBriefing = once(host, 'briefing');
  host.emit('startMatch'); await nextBriefing;
  const cancelled = once(host, 'lobby');
  host.emit('cancelBriefing'); const [cancelledLobby] = await cancelled;
  assert.equal(cancelledLobby.state, 'lobby');
